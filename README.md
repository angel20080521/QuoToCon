...
将 Word 格式的**报价单**自动转换为填写好的**商务合同**。

上传报价单（`.docx`）和合同模板（`.docx`），系统自动提取报价单字段并将其填入合同模板的 `{{占位符}}` 中，生成可直接下载的合同文件。

---

## 功能特性

- 自动从报价单段落（`字段名：值`）和二列表格中提取字段
- 支持全角冒号（`：`）和半角冒号（`:`）
...
报价单（`.docx`）中应包含 `字段名：字段值` 格式的内容，例如：
